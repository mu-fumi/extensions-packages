...
class Book {
  ...
  getPage(type) {
    var curr_page = vscode_1.workspace
      .getConfiguration()
      .get("thiefBook.currPageNumber");
    var page = 0;
    if (type === "previous") {
      if (curr_page <= 1) {
        page = 1;
  ...
  updatePage() {
    ...
    vscode_1.workspace
      .getConfiguration()
      .update("thiefBook.currPageNumber", this.curr_page_number, true);
  ...
  readFile() {
    ...
    var line_break = vscode_1.workspace
      .getConfiguration()
      .get("thiefBook.lineBreak");
    return data
      .toString()
      .replace(/\n/g, line_break)
      .replace(/\r/g, " ")
  ...
  init() {
    this.filePath = vscode_1.workspace
      .getConfiguration()
      .get("thiefBook.filePath");
    var is_english = vscode_1.workspace
      .getConfiguration()
      .get("thiefBook.isEnglish");
    if (is_english === true) {
      this.page_size =
        vscode_1.workspace.getConfiguration().get("thiefBook.pageSize") * 2;
    } else {
      this.page_size = vscode_1.workspace
        .getConfiguration()
        .get("thiefBook.pageSize");
    }
  }
  getPreviousPage() {
    this.init();
